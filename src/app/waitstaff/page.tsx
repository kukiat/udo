...
export default function WaitstaffIndex() {
    ...
    const base = q
      ? branches.filter(
          (b) =>
            ...
            (b.restaurant?.name.toLowerCase().includes(q) ?? false) ||
  ...
  const totalTables = branches?.reduce((s, b) => s + b.tables.length, 0) ?? 0;
  const totalOccupied =
    branches?.reduce((s, b) => s + occupiedOf(b), 0) ?? 0;
  const restaurantsCount = branches
    ? new Set(branches.map((b) => b.restaurant?.id).filter(Boolean)).size
    : 0;
  const liveCount = branches?.filter((b) => b.tables.length > 0).length ?? 0;

  const loading = authLoading || branches === null;
    ...
    >
      ...
      {loading ? (
        ...
        <div style={{ padding: "28px 32px 80px", maxWidth: 1280, margin: "0 auto" }}>
          ...
          >
            <span style={{ color: "var(--ink-2)" }}>Floor</span>
            <span style={{ margin: "0 8px", color: "var(--ink-4)" }}>·</span>
            <span className="tnum">{branches?.length ?? 0} branches</span>
            <span style={{ margin: "0 8px", color: "var(--ink-4)" }}>·</span>
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              >
                Pick a branch to monitor tables, watch order status, and mark
                ready orders as served.
              </div>
            </div>
          ...
          >
            <Stat label="Restaurants" value={String(restaurantsCount)} />
            <Stat label="Branches" value={String(branches?.length ?? 0)} />
            <Stat
              label="Tables"
              value={`${totalOccupied} / ${totalTables}`}
              hint="occupied"
            />
          </div>
          ...
          {!branches || branches.length === 0 ? (
            ...
            >
              <EmptyState
                title="No branches"
                description="There are no branches available to staff yet."
              />
            </div>
          ) : (
            <div
            ...
            >
              <BranchTable
                ...
                sortDir={sortDir}
                onSort={toggleSort}
                onOpen={(id) => router.push(`/waitstaff/${id}`)}
              />
              <Pagination
                page={page}
                pageCount={pageCount}
...
function BranchTable({
  ...
  const cols =
    "52px 56px minmax(220px, 1.8fr) minmax(160px, 1fr) 110px 110px 120px 130px";
  return (
    <div>
      <div
